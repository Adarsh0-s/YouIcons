...
object SvgFilesProcessor {
    ...
    private fun convertToVector(svgSource: Path, vectorTargetPath: Path) {
        if (svgSource.fileName.toString().endsWith(".svg")) {
            ...
            try {
                ...
                updateXmlPath(targetFile, "android:fillColor", bg)
            } catch (e: DocumentException) {
                throw RuntimeException(e)
            }
            createAdaptive(targetFile, getRandomColor())
        } else {
    ...
    private fun getRandomColor(): String {
        return "@color/" + colors[Random.nextInt(colors.size)]
    }
    ...
    private fun updateXmlPath(xmlPath: String, searchKey: String, attributeValue: String) {
        val xmlDocument: Document? = try {
            XmlUtil.getDocument(xmlPath)
        } catch (e: DocumentException) {
            println("Skipping file due to document exception: ${e.message}")
            return
        }

        if (xmlDocument == null) {
            println("Skipping file due to null document")
            return
        }
        ...
        if (xmlDocument.rootElement != null) {
            for (e in xmlDocument.rootElement.elements("path")) {
                ...
                if (attr != null && attr.value != "#00000000") {
